...
// Add product with multiple images (base64)
router.post("/", async (req, res) => {
  try {
    const { name, description, price, images, image, category, subCategory } = req.body;
    ...
    if (images && Array.isArray(images) && images.length > 0) {
      try {
        ...
        const validImages = images.filter(img => {
          return img && (img.base64 || img.data) && 
    ...
    else if (image && typeof image === 'object' && Object.keys(image).length > 0) {
      try {
...
// Add product with multiple file uploads (multipart/form-data)
...
// Add single file upload route for backward compatibility
      ...
      try {
        ...
        if (imageToUpload) {
          ...
          const newImageData = {
            url: cloudinaryResult.url,
            publicId: cloudinaryResult.publicId,
            originalName: image.originalName || image.filename || `${name || product.name}.jpg`,
            uploadedAt: new Date()
          };
